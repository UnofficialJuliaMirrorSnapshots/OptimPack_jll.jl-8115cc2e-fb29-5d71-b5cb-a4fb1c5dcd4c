# Autogenerated wrapper script for OptimPack_jll for x86_64-apple-darwin14
...
const libcobyla_splitpath = ["lib", "libcobyla.1.dylib"]
...
const libcobyla = "@rpath/libcobyla.1.dylib"


# Relative path to `libbobyqa`
const libbobyqa_splitpath = ["lib", "libbobyqa.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libbobyqa = "@rpath/libbobyqa.1.dylib"


# Relative path to `libopk`
const libopk_splitpath = ["lib", "libopk.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libopk = "@rpath/libopk.2.dylib"


# Relative path to `libnewuoa`
const libnewuoa_splitpath = ["lib", "libnewuoa.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnewuoa = "@rpath/libnewuoa.1.dylib"
